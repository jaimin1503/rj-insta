...
function Leftnav({user}) {
    ...
    return (
        <div className={` w-[240px] overflow-y-auto h-full py-4 text-black z-10 translate-x-[-240px] sm:translate-x-0 transition-all border-r-2`}>
            <div className="flex px-5 flex-col mx-auto gap-4    ">
            <img src={instagramlogo} className='bg-white h-10 w-24 object-contain '></img>
            {   
            
                categories.map((navitem) => {
                    return <Leftnavitem icon={navitem.icon} name={navitem.name} action={() => {
